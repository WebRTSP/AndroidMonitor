    ...
    override fun onCreate() {
        ...
        lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                val reStreamerSettings = settingsRepository.reStreamerSettingsFlow.first()
                with(reStreamerSettings) {
                    ReStreamer(serverUrl, clientId, credentials)
                }.use { reStreamer ->
                    reStreamer.state.first { it == ReStreamer.State.Connected }

                    launch {
                        reStreamer.credentials.filterNotNull().collect { credentials ->
                            settingsRepository.setReStreamerCredentials(credentials)
                        }
                    }
