    ...
    _onStateChangedJni(GetMethodID(env, oppositeBank, "onStateChangedJni", "(I)V"))
{
    ...
    static thread_local std::weak_ptr<ActorContext> sharedActorContext;

    if(sharedActor.expired()) {
        _actorContext = std::make_shared<ActorContext>(_javaVm);
        sharedActorContext = _actorContext;
        _actor = std::make_shared<Actor>(_actorContext);
        sharedActor = _actor;
    } else {
        _actorContext = sharedActorContext.lock();
...
void GStreamerPlayer::onStateChanged(State state) noexcept {
    _actorContext->actorJniEnv->CallVoidMethod(
        _oppositeBank,
        _onStateChangedJni,
...
void GStreamerPlayer::stop() noexcept {
    ...
    _actor->sendAction([pipeline = _pipeline] () {
